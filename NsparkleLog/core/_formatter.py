...
class Formatter:
    ...
    def format(self, 
        ...
        formatted_msg = self._fmt.format(
            ...
            levelcolor=f"<{LevelColor.getlevelColor(record.level)}>" if self.colorMode else "", # type: ignore
            levelColorEnd=f"</{LevelColor.getlevelColor(record.level)}>" if self.colorMode else "" # type: ignore
        )
        if self.colorMode:
